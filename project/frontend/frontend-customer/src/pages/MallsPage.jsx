...
// ✅ ไว้หาไฟล์รูปแผนที่จากฝั่ง admin
// ถ้าพอร์ต admin ของคุณไม่ใช่ 5173/4173 ค่อยเปลี่ยนตรงนี้
const ADMIN_ASSET_ORIGINS = [
  '',
  'http://localhost:5173',
  'http://127.0.0.1:5173',
  'http://localhost:4173',
...
function getFloorImageCandidates(floorCode) {
  const code = String(floorCode || 'G').toUpperCase();
  const fileMap = {
    LG: 'LG.png',
    G: 'G.png',
    '1': '1.png',
  ...
  const fileName = fileMap[code] || 'G.png';
  return ADMIN_ASSET_ORIGINS.map((origin) => `${origin}/picture/${fileName}`);
}
...
export default function MapPage() {
  ...
  const [imageTryIndex, setImageTryIndex] = useState(0);
  ...
  const mapCandidates = useMemo(() => {
    return getFloorImageCandidates(activeFloor?.floor_code);
  }, [activeFloor?.floor_code]);

  const currentMapImage = mapImageSrc || mapCandidates[imageTryIndex] || '';
        ...
        >
          ...
          >
            {currentMapImage ? (
              <img
                src={currentMapImage}
                alt={`Floor ${activeFloor?.floor_code || 'G'}`}
                draggable={false}
                onLoad={(e) => {
                  if (e.currentTarget.src !== mapImageSrc) {
                    setMapImageSrc(e.currentTarget.src);
                  }
                }}
                onError={() => {
                  if (imageTryIndex < mapCandidates.length - 1) {
                    setImageTryIndex((prev) => prev + 1);
                  }
                }}
                style={{
                  width: '100%',
